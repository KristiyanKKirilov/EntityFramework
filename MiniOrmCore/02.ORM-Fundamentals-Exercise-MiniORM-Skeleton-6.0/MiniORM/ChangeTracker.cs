...
namespace MiniORM
{
    public class ChangeTracker<T> where T : class, new()
    {
        ...
        {
            var trackedProperties = typeof(T)
                .GetProperties()
        ...
        private List<T> CloneEntities(IEnumerable<T> entities)
        {
            ...
            PropertyInfo[] properitesToClone = typeof(T)
                .GetProperties()
                .Where(pi => DbContext.AllowedSqlTypes.Contains(pi.PropertyType))
                .ToArray();
            ...
            foreach (var entity in entities)
            {
                ...
                foreach (var currentProperty in properitesToClone)
                {
                    var value = currentProperty.GetValue(entity);
                    currentProperty.SetValue(clonedEntity, value);
                }
